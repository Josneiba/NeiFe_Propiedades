...
export function generatePaymentReminderEmailHTML(params: PaymentReminderParams): string {
  ...
  return buildBrandedEmailHtml({
    preview: `Recordatorio de pago de arriendo · ${params.month}`,
    title: 'Recordatorio de pago',
    greeting: `Hola ${params.tenantName},`,
    intro: [
      `Te recordamos que el pago de arriendo correspondiente a ${params.month} está próximo a vencer.`,
    ],
    infoRows: [
      { label: 'Propiedad', value: params.propertyAddress },
      { label: 'Monto a pagar', value: `$${formattedAmount}` },
      { label: 'Fecha de vencimiento', value: formattedDueDate },
    ],
    bulletList: [
      '<strong>Tarjeta de crédito o débito</strong> para pago inmediato.',
      '<strong>Transferencia bancaria</strong> con los datos disponibles en tu panel.',
    ],
    cta: {
      label: 'Revisar pago en NeiFe',
      url: 'https://neife.cl/mi-arriendo/pagos',
    },
    closing: [
      'Si ya realizaste el pago, puedes ignorar este recordatorio.',
      'Si necesitas ayuda, escríbenos a soporte@neife.cl.',
    ],
  })
}
...
export function generateLandlordNotificationHTML(params: LandlordNotificationParams): string {
  return buildBrandedEmailHtml({
    preview: `Pagos pendientes en ${params.propertyAddress}`,
    title: 'Pagos pendientes por revisar',
    greeting: `Hola ${params.landlordName},`,
    intro: [
      `Detectamos ${params.pendingCount} propiedad${params.pendingCount === 1 ? '' : 'es'} con pagos pendientes que requieren atención en tu panel.`,
    ],
    infoRows: [
      { label: 'Propiedad destacada', value: params.propertyAddress },
      { label: 'Total pendiente', value: `$${params.totalDue.toLocaleString('es-CL')}` },
    ],
    cta: {
      label: 'Revisar pagos en NeiFe',
      url: 'https://neife.cl/dashboard/pagos',
    },
    closing: [
      'Te recomendamos revisar el detalle para tomar acción con tiempo.',
    ],
  })
}
