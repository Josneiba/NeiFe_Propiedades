...
export default async function MetricasPage() {
  ...
  const [
    ...
    prisma.crmDeal.findMany({
      ...
      include: {
        ...
        contacts: { include: { contact: { select: { code: true, name: true } } }, take: 2 },
      },
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="bg-[#1C2828] border border-[#D5C3B6]/10 rounded-xl p-4">
        ...
        <div className="space-y-3">
          {dealsByPhase.map((item) => {
            const phaseLabel = 
              item.phase === 'PRE_VENTA' ? 'Pre-Venta' :
              item.phase === 'VENTA' ? 'Venta' :
              item.phase === 'POST_VENTA' ? 'Post-Venta' : item.phase
            const phaseColor =
              item.phase === 'PRE_VENTA' ? '#1a3a5c' :
              item.phase === 'VENTA' ? '#0e4d3a' :
              item.phase === 'POST_VENTA' ? '#4a1a5c' : '#5E8B8C'
            return (
              <div key={item.phase} className="flex items-center gap-4">
                <span className="text-xs text-[#9C8578] w-24">{phaseLabel}</span>
      ...
      <div className="bg-[#1C2828] border border-[#D5C3B6]/10 rounded-xl p-4">
        <h2 className="text-sm font-semibold text-[#FAF6F2] mb-1">⚡ Próximas acciones</h2>
        <p className="text-xs text-[#9C8578] mb-4">Lo que necesitas hacer hoy para no perder oportunidades</p>
        ...
        <div className="space-y-2">
          ...
          {staleDeals.map((deal) => {
            ...
            return (
              <a key={deal.id} href={`/broker/crm/workspace`}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-[#FAF6F2] font-medium">Contactar sobre {deal.title}</div>
                    <div className="text-[#9C8578] text-[9px] mt-0.5">
                      Deal en <strong>{deal.phase === 'VENTA' ? 'Venta' : 'Post-Venta'}</strong> sin actividad reciente
                      {mainContact && ` — contactar a ${mainContact.contact.name}`}
                    </div>
                  </div>
                </div>
          ...
          {unpairedTenants.map((c) => (
            <a key={c.id} href={`/broker/crm/contactos/${c.id}`}
              className="flex items-center justify-between text-xs bg-[#2D3C3C]/60 rounded-lg px-3 py-2.5 hover:bg-[#2D3C3C] transition-colors group">
              <div className="flex items-center gap-3">
                <div className="text-lg">🔍</div>
                <div>
                  <div className="text-[#FAF6F2] font-medium">Asignar propiedad a [{c.code}] {c.name}</div>
                  <div className="text-[#9C8578] text-[9px] mt-0.5">
                    Arrendatario activo sin propiedad vinculada — no puede avanzar en el funnel
                  </div>
                </div>
              </div>
              <div className="text-[#9C8578] group-hover:text-[#D5C3B6]">→</div>
          ...
          {staleDeals.length === 0 && unpairedTenants.length === 0 && (
            <div className="text-center py-6">
              <p className="text-xs text-[#9C8578]">🎯 Sin acciones urgentes por ahora — buen trabajo</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="space-y-2">
          {topRisks.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-xs text-[#9C8578]">✅ Todos los contactos están al día</p>
            </div>
          ) : (
            topRisks.map((s) => (
              <a key={s.id} href={`/broker/crm/contactos/${s.contactId}`}
                ...
                <div className="flex items-center gap-3">
                  {/* Score visual circle */}
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center font-semibold text-[9px] ${
                    s.score < 30 ? 'bg-red-500/20 text-red-400' :
                    s.score < 50 ? 'bg-yellow-500/20 text-yellow-400' :
                  ...
                  <div>
                    ...
                    <div className={`text-[9px] mt-0.5 ${
                      s.lastActivityDays > 7 ? 'text-red-400' :
                      s.lastActivityDays > 3 ? 'text-yellow-400' : 'text-green-400'
                    }`}>
                      {s.lastActivityDays === 999 ? 'Sin actividad' : `${s.lastActivityDays}d sin contacto`}
                    </div>
                  </div>
                </div>
                {s.recommendation && (
