...
export function PipelineTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && selected && selected.items.length === 0 && (
        <div className="rounded-lg border border-[#2D3C3C] bg-[#1a2a2a] p-6 text-center text-[#D5C3B8]">
          <p className="text-sm font-semibold text-[#FAF6F2]">No hay registros para este filtro.</p>
          <p className="mt-2 text-sm text-[#9C8578]">Este segmento aún no tiene datos visibles en el pipeline. Ve al workspace CRM para revisar la lista completa de deals y activar los filtros.</p>
          <Link href="/broker/crm/workspace" className="mt-4 inline-flex rounded-lg bg-[#5E8B8C] px-4 py-2 text-sm font-semibold text-[#FAF6F2] hover:bg-[#5E8B8C]/90">
            Ir al workspace CRM
          </Link>
        </div>
      )}
